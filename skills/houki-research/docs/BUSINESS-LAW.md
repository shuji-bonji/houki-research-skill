...
このスキルが引き受けるのは、**士業者に相談するまでの情報整理** です。相談の代わりではありません。利用者が有資格者のところへ持って行けるように、関わる条文と、拘束力の別と、何が事実認定に依存するかまでを揃えます。結論・可否・金額は、資格を持つ人が事案を見て出すものなので返しません。
...
判定をどこに置くかという観点では、当てはめは **系の外** にある。[houki-hub](https://github.com/shuji-bonji/houki-hub) は判定の行き先を 4 つに分けており (コード / 判定不能として返す / LLM (選ばない) / 系の外)、当てはめはその 4 つ目にあたる。境界 ② の [応答型](#応答型) は、その 4 つ目に対応する **出力の形** を定める。
...
graph TB
  ...
  classDef ng fill:#f8d7da,stroke:#dc3545,color:#333
```
...
境界 ② の問いに対して、返してよいものと返さないものを次のとおり固定する。

| | 内容 |
| --- | --- |
| **返すもの** | 関連する条文・通達・裁決の提示 (citation 付き) / 制度の概観と改正履歴 / 論点の列挙 / 何が事実認定に依存するかの明示 / `legal_status` の階層 |
| **返さないもの** | 結論 (該当する・しない) / 可否の判定 / 金額・税額の確定 / 書類の起案・文案 / 「おそらく〜でしょう」を含む推測 |

**「返さないもの」は、注意喚起を添えても返さない。** §4 の注意喚起テンプレートは、返してよいものの書き方を定めるものであり、返さないものを返せるようにするものではない。

「何が事実認定に依存するか」は返す側にある。「この扱いは、当該取引が国内取引に当たるかで変わります」は事実の所在を示す文であって、結論ではない。**どちらに転ぶかを書いた時点で結論になる。**

#### なぜ返さないのか — 基準が書けないからではない

当てはめの基準は、条文と通達に書いてある。**書けないから返さないのではない。** 返さない理由は §1 の独占規定である。

この区別を保つ理由は 1 つ。「資料が揃えば、将来はツールが結論を返してよくなる」という読みを残さないためである。独占規定が変わらない限り、当てはめは資料が揃っても系の外にある。
...
- 境界 ② の応答型を変えるときは、[houki-hub](https://github.com/shuji-bonji/houki-hub) の「判定の置き場所は 3 つではなく 4 つ」と語彙を揃える
