...
houki-egov-mcp v0.6.x には委任先を自動で辿るツールが無い（houki-egov-mcp#20 で予定）。それまでは名称の規則と `law_type` で引く。
...
`examples/electronic-bookkeeping.md`（予定）。電子帳簿保存法の電子取引を題材に、ステップ ②〜⑦ の実測を残す。
